...
template <typename T>
class WhereObserver : public Observable<T>, public IObserver<T>
{
public:
	WhereObserver(std::function<bool(T)>);
...
template <typename T>
WhereObserver<T>::WhereObserver(std::function<bool(T)> predicate) :
	predicate(predicate) {}

template <typename T>
void WhereObserver<T>::OnNext(T data)
{
	try
	{
		if (predicate(data))
...
template <typename T>
void WhereObserver<T>::OnComplete()
{
	try
	{
		for (auto iter = this->observers.begin(); iter != this->observers.end(); iter++)
...
template <typename T>
void WhereObserver<T>::OnError(std::exception e)
{
	for (auto iter = this->observers.begin(); iter != this->observers.end(); iter++)
	{
		(*iter)->OnError(e);
